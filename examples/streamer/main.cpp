...
static void USER_CODE_BEGIN() {
    testSDCARD.setURL(TEST_SDCARD_URL);
    testSDCARD.setSDState(SDCard::eSDDetect::Inserted);
}
...
int main(int argc, char **argv) try {
    ...
    string localId = "camerainfake0134";
    ...
    const string url = "ws://42.116.138.38:8089/" + localId;
...
shared_ptr<Client> createPeerConnection(const Configuration &config,
    ...
    pc->onStateChange([id](PeerConnection::State state) {
        cout << "PeerConnection  State: " << state << endl;
        if (state == PeerConnection::State::Disconnected ||
            state == PeerConnection::State::Failed ||
            state == PeerConnection::State::Closed) {
            // remove disconnected client
            std::cout << "Client " << id << "has removed." << std::endl;
    ...
    client->audio = addAudio(pc, 8, 2, "audio-stream", "stream1", [id, wc = make_weak_ptr(client)]() {
        MainThread.dispatch([wc]() {
            if (auto c = wc.lock()) {
                addToStream(c, false);
            }
    ...
    dc->onMessage(nullptr, [id, wdc = make_weak_ptr(dc)](string msg) {
        ...
        string resp = "";
        UsrDataChannelHdl(id, msg, resp); // sau khi chay xong thi chuyen gi se xay ra

        if (auto dc = wdc.lock()) {
            std::cout << "DataChannel send: " << resp.data() << std::endl;
    ...
    stream->onSample([ws = make_weak_ptr(stream)](Stream::StreamSourceType type, uint64_t sampleTime, rtc::binary sample) {
        ...
        if (!tracks.empty()) {
            for (auto clientTrack: tracks) {
                ...
                double n = rtpConfig->timestampToSeconds(reportElapsedTimestamp);
                ...
                if (sample.size() > 0) {
                    // cout << "Sending " << streamType << " sample with size: ";
                    // cout << to_string(sample.size()) << " to " << client;
                    // cout << ", Spending " << std::fixed << std::setprecision(5) << elapsedSeconds << " secs" << endl;
